...
import reidc.xyz.smartpass.Screens.ScannerFragment;
import reidc.xyz.smartpass.Screens.SignInFragment;
...
public class MainActivity extends AppCompatActivity
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if(UserHandler.i().getCurrentUser()!=null) {
            // Logged in already
            if (id == R.id.nav_camera) {
                Frame.switchFrame(getFragmentManager(), new ScannerFragment());

            } else if (id == R.id.nav_manage) {
                User user = UserHandler.i().getCurrentUser();
                UserHandler.i().setTargetUser(user.getEmail(), user.getPassword());

                Frame.switchFrame(getFragmentManager(), new ProfileScreen());
            }
        } else {
            Toast.makeText(getApplication(), "You must be logged in!", Toast.LENGTH_LONG);
        }
        DrawerLayout drawer = (DrawerLayout) findViewById(R.id.drawer_layout);
        drawer.closeDrawer(GravityCompat.START);
        return true;
